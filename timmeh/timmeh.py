...
DISTANCE = CONVERSION['nautical'] * 12

geoid = pyproj.Geod(ellps='WGS84')  # See pyproj documentation for other fun options
...
def readit(lat, lon):
    ...
    color_values = pix[px, py]
    if color_values in bigdic.keys():
        tz = bigdic[color_values]
        print('Lat:', lat, ' Lon:', lon)
        print('Timezone:', tz, 'px:', px, 'py:', py)
    else:
        if color_values in seadic.keys():  # If pixel is ocean,
            for bearing in range(0, 315, 45):  # look at 8 points around the gps location.
                lon, lat = high_seas(lat, lon, bearing, DISTANCE)  # return those points
                x, y = coordinates(lon, lat)  # projection calculations
                px = int((x + pi) * ((size_x / 2) / pi))  # for pixel location
                py = int(size_y - (y + (pi / 2)) * (size_y / pi))  # on both axises
                color_values = pix[px, py]  # and give the color value
                try:
                    if color_values not in seadic.keys():  # it's probably close enough to land
                        tz = bigdic[color_values]  # give that.
                        print('Lat:', lat, ' Lon:', lon)
                        print('Timezone:', tz, 'px:', px, 'py:', py)
                except (OSError, IOError) as error:  # bad bad pixels
                    # in the color spread of 35/polygon on 0-255 scale X RGB
                    print('\r\nTHIS erprr: {}'.format(error))  # Slap them in line
                    color_values = tuple(round(value / color_spread) * color_spread for value in color_values)
                    try:  # to try again
                        if color_values not in seadic.keys():  # if it's not at sea
                            tz = bigdic[color_values]  # use that.
                            print('Lat:', lat, ' Lon:', lon)
                            print('Timezone:', tz, 'px:', px, 'py:', py)
                    except:  # when it's just too edge case to ponder.
                        print("Lasciate ogne speranza, voi ch'intrate")


def high_seas(lat, lon, azimuth, distance):  # Because I can't think of a better name right now.
    ...
    Returns:
        longitude (float): longitude of solution
        latitude (float: latitude of solution
     #  __bearing_fro (float): bearing from the solution point to the reference point
    """
    longitude, latitude, __bearing_fro = geoid.fwd(lat, lon, azimuth, distance)
